...
- A persistent fleet toolbar in the hidden navigation provides quick filtering, Smart-first sorting, refresh cadence, a column chooser, resizable row columns, and the Cards/Rows switch.
...
- A full-width inline detail panel places system identity, uptime, all current utilization, and every available historical telemetry category directly before the ranked grid.
...
Cards establish an inline-size container so header type and utilization-track thickness respond to the actual card width rather than only the viewport. Every CSS-grid row stretches its cards to the tallest card in that row, so systems without extra drives retain a complete aligned card surface. The navigation's fleet toolbar stores its layout, filter, sort, refresh, visible-column, and width preferences locally. Smart risk priority remains the default, while explicit alternatives support system name, CPU, RAM, fullest drive, and uptime. Refresh begins from the deployment setting and may be changed to manual, 15 seconds, 30 seconds, 1 minute, or 5 minutes; both the fleet and open history reload on the selected cadence. Row mode uses a compact table with one 38px server row and resizable columns for identity, condition, CPU, RAM, and drives; identity, condition, tracks, labels, and values all stay in one horizontal band. Extra drives remain on that same row and overflow horizontally inside their cell when necessary. Below 1120px, the hidden toolbar wraps controls according to available space instead of overflowing the viewport. At 720px, the quick filter takes a dedicated line while the summary becomes three count cells with refresh spanning the next row. At 520px, the card grid becomes a single full-width column, navigation switch labels disappear while its icons retain 44px touch targets, the external action becomes icon-only, gauges stack, and range controls stretch across the detail panel. The row table preserves its compact desktop geometry inside deliberate horizontal overflow, with synchronized scrollbars above and below the fleet so user-sized columns remain immediately reachable. The history plot keeps a 520px minimum width inside deliberate horizontal overflow.

Clicking anywhere on a card or row inserts a full-width detail panel between the priority heading and the fleet. The system-name control remains the keyboard entry point. The panel participates in normal document flow, remains nonmodal, and leaves every fleet item available below it. It shows identity, uptime, current CPU, RAM, and all reported drives above a responsive grid of whichever historical categories the system reports. Closing the panel removes that inspection region without changing risk order.
...
The top dock contains a low product header and a dense fleet toolbar. On mouse and trackpad systems the entire dock remains completely off-canvas until the pointer reaches the top 22px or focus enters a control, then arrives as a fixed overlay with a short ease-out transition. It hides again when pointer and focus leave; pointer exit also clears mouse-retained focus and closes open toolbar disclosures, while keyboard users retain focus-driven access. The column chooser additionally dismisses on outside press and Escape, and its panel opens from the trigger's left edge so wrapped layouts never send it beyond the viewport. Touch systems keep it visible so navigation never depends on hover. The toolbar owns the Fleet priority label, quick filter, sort selector, refresh selector, column chooser, and Cards/Rows switch so the resting dashboard spends no vertical space on secondary controls. Keyboard-accessible separators resize visible row columns. The brand index uses cobalt mono text; product name and actions remain neutral. Below 520px the view labels disappear while their authored grid/row icons remain in 44px targets, and the external action becomes icon-only.
...
Row mode is a semantic table with one server per 38px row and stable System, Condition, CPU, RAM, and Drives columns. Compact utilization tracks are 14px tall so the progress bars stay prominent inside the compact row. The system-name control opens the same inline detail panel used by cards. CPU and RAM place their short semantic track and value side by side; every reported drive places a compact, emphasized drive label, track, and value on that same line inside one nonwrapping drives cell. Normal systems leave Condition empty; only actionable or exceptional states earn text. The table scrolls horizontally on narrow viewports instead of collapsing columns or creating multi-line server rows, and its synchronized top scrollbar prevents right-side columns from becoming undiscoverable in long fleets.
